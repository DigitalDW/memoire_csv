...
for subdir, dirs, files in os.walk(rootdir):
    for file in files:
        ...
        if 'THINGS' in currFile:
            newFile = subdir.removeprefix('.') + '_thing_data.txt'
            with open(currFile, 'rb') as inputFile:
                data = inputFile.read()
                final = 'episode mission x_position y_position direction type flags\n'
                offsets = [2, 2, 2, 2, 2]
                compteur = 0
                compteurOffset = 0
